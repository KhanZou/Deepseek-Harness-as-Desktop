...
> **路径约定（v0.x）**：插件与 exe 当前假定
> `D:\deepseek harness`、`D:\dsh-desktop-window`、`D:\dsh-desktop-settings`。
> exe 支持 `--workdir/--url/--port/--width/--height` 覆盖；宿主插件读取
> `DSH_DESKTOP_URL/WIDTH/HEIGHT/AUTO` 环境变量。
