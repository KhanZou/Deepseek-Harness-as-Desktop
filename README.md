...
> **Path assumptions (v0.x):** the plugins and the exe currently assume
> `D:\deepseek harness`, `D:\dsh-desktop-window`, and `D:\dsh-desktop-settings`.
> The exe accepts `--workdir/--url/--port/--width/--height` overrides; the
> host plugin reads `DSH_DESKTOP_URL/WIDTH/HEIGHT/AUTO` env vars.
