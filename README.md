...
- ✨ **Interactive toasts**
- 🧩 **`dsh-skill-manager` plugin** — per-workspace skill manager (enable/disable project & global skills, copy/apply skill config between projects); ships as an external plugin depending on the published `@deepseek-ai/dsh-*` workspace packages.
- 📱 **QQ notifications + remote conversation** — step-by-step setup guide for `dsh-notifier` (outbound push to QQ, inbound remote approval/dialogue, the pitfalls we actually hit): [`docs/qq-notifier-setup.md`](docs/qq-notifier-setup.md).
