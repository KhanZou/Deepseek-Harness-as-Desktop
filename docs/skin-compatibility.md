...
This project's **Skin Center** (its own Settings tab — Settings → 皮肤中心 /
Skin Center — ordered after all native settings tabs) discovers skins from the
installed DSH profile and lets the user pick **one** of them (or the built-in
default). This page documents the conventions a third-party skin
...
`D:\dsh-desktop-window\skins.json`, which the Skin Center reads via
`GET /api/skins`. A built-in `default` entry is always prepended. Besides the
identity fields below, the manifest now passes through `accent`, `tags` and
`bodyAttr` from `skin.json` so the Skin Center can paint accurate previews.
...
| `accent` | Optional brand/accent color (e.g. `"#c5a468"`) — used to paint the mini mockup preview |
| `tags` | Optional display tags (e.g. `["anime"]`) — carried into the manifest |
| `bodyAttr` | Optional body data-attribute the skin sets — carried into the manifest |
...
Restart the desktop app (the manifest is regenerated by the `dsh-desktop-window`
plugin on boot). The skin then appears in the Skin Center picker. Each picker
card shows a mini app-window mockup painted with the **live theme tokens**; when
the skin provides an `accent`, that color is used for the mockup's brand
elements (active nav item, send button, own chat bubble). If
`preview.light` / `preview.dark` are present they are shown instead.
